...
int clock_twi_onoff(int port, int state)
{
	...
	if (state) {
		setbits_le32(&ccm->twi_gate_reset, 1UL << (GATE_SHIFT + port));
		setbits_le32(&ccm->twi_gate_reset, 1UL << (RESET_SHIFT + port));
	} else {
		clrbits_le32(&ccm->twi_gate_reset, 1UL << (GATE_SHIFT + port));
		clrbits_le32(&ccm->twi_gate_reset, 1UL << (RESET_SHIFT + port));
	}
